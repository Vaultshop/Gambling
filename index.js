// ============ KONFIGŪRACIJA ============
const config = {
  token: 'ĮDĖK_SAVO_TOKENĄ',
  ownerId: 'TAVO_DISCORD_ID',
  adminRoleId: null, // admin rolės ID (null = adminai = owner + Administrator teisė)
  ticketCategoryId: null, // kategorijos ID ticketams (null = kuriasi pagrindame)
  brand: 'Ticket',
...
const pendingPhoto = new Map(); // channelId -> { info, note, adminId }
...
function panelEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription(
      'Paspausk mygtuką pagal poreikį:\n\n' +
...
client.once('ready', async () => {
  ...
  await new REST({ version: '10' }).setToken(config.token).put(Routes.applicationCommands(client.user.id), { body: commands });
  ...
  try {
    ...
    if (i.isChatInputCommand()) {
      ...
      if (i.commandName === 'panel') {
        return i.reply({ embeds: [panelEmbed()], components: panelRow() });
      }
    }
    ...
    if (i.isModalSubmit() && i.customId === 'm_product') {
      ...
      pendingPhoto.set(i.channel.id, data);
      ...
      channel.awaitMessages({ filter: m => m.author.id === data.adminId, max: 1, time: 60000 })
        .then(col => {
          pendingPhoto.delete(channel.id);
          const m = col.first();
          let image = null;
          if (m) {
            if (m.attachments.size) image = m.attachments.first().url;
            m.delete().catch(() => {});
          }
          return sendProduct(channel, data, image);
        })
        .catch(() => { pendingPhoto.delete(channel.id); return sendProduct(channel, data, null); });
    }
  } catch (e) {
    console.error(e);
